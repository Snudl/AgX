This is an OCIO config adapted from [EaryChow AgX](https://github.com/EaryChow/AgX) and [EaryChow LUT Gen](https://github.com/EaryChow/AgX_LUT_Gen). In addition to all the existing definitions it adds more targeted `AgX - HDR` presets to `Rec.2100-PQ`, `Rec.2100-HLG`, and `Extended sRGB` under `Display`, outputting floating-point color values above 1.0 for HDR brightness beyond the SDR peak white reference and negative values for achieving wider than sRGB gamut colors.
